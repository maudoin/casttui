...
public:
  using MediaStatus = DownCastStorage::MediaStatus;

  DowncastLogic(std::string const& dbPath);
  ~DowncastLogic();
  ...
  bool isStatusActive(std::optional<DowncastLogic::MediaStatus> const& status)const;
...
private:
  ...
  void setLastError(DownCastStorage::Result const& result);
  ...
  MutexedData<DownCastStorage> m_storage;
