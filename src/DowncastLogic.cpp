...
DowncastLogic::DowncastLogic(std::string const& dbPath)
  : m_dbPath(dbPath)
  , m_storage(m_dbPath)
{
  ...
  {
    DownCastStorage::initDefaultContent(dbPath);
...
//-----------------------------------------------------------------------------------
void DowncastLogic::restartIf(int podcastId)
{
...
//-----------------------------------------------------------------------------------
bool DowncastLogic::noPodcastFilter() const
{
  return !m_currentPodcastId;
}
//-----------------------------------------------------------------------------------
const char* DowncastLogic::podcastTitle(int i) const
{
  auto const& p{m_podcasts[i]};
  return p.title.empty()?p.link.c_str():p.title.c_str();
}
//-----------------------------------------------------------------------------------
bool DowncastLogic::setCurrentPodcastRowIndex(std::optional<std::optional<int>> const& i,
...
//-----------------------------------------------------------------------------------
bool DowncastLogic::isCurrentPodcast(int i) const
{
...
//-----------------------------------------------------------------------------------
std::vector<MediaViewCols>const& DowncastLogic::showsInRankRange(int iStart, int count)
{
  return showsInRankRange(m_displayedShowRangeCache, iStart, count);
}
//-----------------------------------------------------------------------------------
std::vector<MediaViewCols>const&
DowncastLogic::showsInRankRange(ShowRangeDisplayCache& cache, int iStart, int count)
{
  if(!m_busy &&
...
DowncastLogic::showsInRankRange(int iStart, int count)
{
...
//-----------------------------------------------------------------------------------
bool DowncastLogic::isShowRankSelected(int iRank) const
{
...
//-----------------------------------------------------------------------------------
void DowncastLogic::selectShowRange(int rankFirst, int rankLast, bool addToSelection)
{
  if(!addToSelection)
  {
    std::fill(m_selectedShowRanks.begin(), m_selectedShowRanks.end(), false);
...
//-----------------------------------------------------------------------------------
void DowncastLogic::refresh(std::string const& url, int const podcastId, bool updatePodcast)
{
...
DowncastLogic::queryPodcast(std::string const& url)const
{
...
void DowncastLogic::writeStorage(OP const& writeOp)
{
  m_busy=true;
  if(m_work)
  {
...
//-----------------------------------------------------------------------------------
bool DowncastLogic::insertPodcast(PodcastCols const& podcast, InsertPodcastMode mode)
{
...
//-----------------------------------------------------------------------------------
void DowncastLogic::refreshCurrentPodcast()
{
...
//-----------------------------------------------------------------------------------
void DowncastLogic::refreshPodcastAtIndex(int index)
{
...
//-----------------------------------------------------------------------------------
void DowncastLogic::setSelectedShowsStatus(Status status)
{
  auto ids = [&]{
    auto storage = m_storage.lock();
...
//-----------------------------------------------------------------------------------
bool DowncastLogic::isBusy()const{return m_busy;}
//-----------------------------------------------------------------------------------
int DowncastLogic::showCount()const{return m_showCount;}
//-----------------------------------------------------------------------------------
int DowncastLogic::podcastCount()const{return m_podcasts.size();}
//-----------------------------------------------------------------------------------
PodcastCols const& DowncastLogic::podcast(std::size_t const i)const{return m_podcasts[i];}
//-----------------------------------------------------------------------------------
bool DowncastLogic::isStatusActive(std::optional<DowncastLogic::MediaStatus> const& status)const
{
  auto storage = m_storage.lock();
  return status == storage->getCurrentStatusFilter();
}
//-----------------------------------------------------------------------------------
bool DowncastLogic::isDownloading()const
{
  return m_downloading;
}
//-----------------------------------------------------------------------------------
DowncastLogic::DownloadInfo DowncastLogic::getCurrentDownloadProgress()const
{
...
//-----------------------------------------------------------------------------------
std::string DowncastLogic::computeBaseFilename(const char* pattern,
...
//-----------------------------------------------------------------------------------
std::vector<PodcastCols> DowncastLogic::retrieveCurrentPodcastsCopy()const
{
  if(m_currentPodcastId)
  {
...
//-----------------------------------------------------------------------------------
void DowncastLogic::abortDownload()
{
  m_abortDownload.store(true);
}

//-----------------------------------------------------------------------------------
void DowncastLogic::startDownload()
{
  std::vector<PodcastCols> podcasts = retrieveCurrentPodcastsCopy();
  if(!podcasts.empty())
  {
...
//-----------------------------------------------------------------------------------
void DowncastLogic::resetLastError()
{
  auto error = m_lastError.writeLock();
  error->clear();
}
//-----------------------------------------------------------------------------------
void DowncastLogic::setLastError(std::string const& msg)
{
  auto error = m_lastError.writeLock();
  *error = msg;
}
//-----------------------------------------------------------------------------------
void DowncastLogic::setLastError(DownCastStorage::Result const& result)
{
  if(result)
  {
    resetLastError();
...
//-----------------------------------------------------------------------------------
std::string const* DowncastLogic::lastError()const
{
