...
#include "DownCastStorage.h"
#include "SQLiteStorageEngine_t.h"
...
DownCastStorage::DownCastStorage(std::string const& dbPath)
  :m_engine(dbPath)
{
}
//-----------------------------------------------------------------------------------
std::vector<PodcastCols> DownCastStorage::readPodcasts()const
{
  return m_engine.readTable<PodcastCols>();
}
//-----------------------------------------------------------------------------------
bool DownCastStorage::setFilter(std::optional<MediaStatus> const& status)
{
...
//-----------------------------------------------------------------------------------
std::optional<DownCastStorage::MediaStatus> DownCastStorage::getCurrentStatusFilter()const
{
  return m_mediaStatus;
}
//-----------------------------------------------------------------------------------
std::vector<DownCastStorage::Filter> DownCastStorage::filters(int podcastId)const
{
...
//-----------------------------------------------------------------------------------
std::vector<DownCastStorage::Filter> DownCastStorage::filters(std::optional<int> podcastId)const
{
  if(podcastId)
  {
...
//-----------------------------------------------------------------------------------
std::vector<MediaViewCols> DownCastStorage::emissions(std::optional<int> podcastId, int iStart, int count)
{
  return m_engine.readTable<MediaViewCols>(filters(podcastId), m_engine.sorting(), iStart, count);
}
//-----------------------------------------------------------------------------------
std::vector<MediaCols> DownCastStorage::queryDownloads(std::optional<int> podcastId) const
{
...
//-----------------------------------------------------------------------------------
bool DownCastStorage::isOpen()const
{
  return m_engine;
}
//-----------------------------------------------------------------------------------
bool DownCastStorage::open()
{
  return m_engine.open(SQLiteStorageCreationEngine::Mode::ReadWrite);
}
//-----------------------------------------------------------------------------------
void DownCastStorage::close()
{
  m_engine.close();
}
//-----------------------------------------------------------------------------------
std::vector<int>
DownCastStorage::getShowsIds(std::optional<int> const podcastId, std::vector<bool>const& mask) const
{
...
//-----------------------------------------------------------------------------------
template <typename T>
SqlInserter<T> DownCastStorage::buildInserter(SqlInserterMode const mode)
{
  return m_engine.buildInserter<T>(mode);
}
template SqlInserter<MediaCols> DownCastStorage::buildInserter(SqlInserterMode mode);
template SqlInserter<PodcastCols> DownCastStorage::buildInserter(SqlInserterMode mode);

//-----------------------------------------------------------------------------------
DownCastStorage::Result
DownCastStorage::setShowsStatus(std::vector<int>const& ids, Status const status)
{
...
//-----------------------------------------------------------------------------------
DownCastStorage::Result
DownCastStorage::deletePodcast(int const podcastId)
{
  return m_engine.deleteRecords(SQLPodcastCols::table,SQLPodcastCols::id, {podcastId});
}
//-----------------------------------------------------------------------------------
DownCastStorage::Result DownCastStorage::initDefaultContent(const std::string &dbPath)
{
